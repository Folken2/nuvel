"""
scaffold.py — The Anthropic Managed Agents Stamping Script

Copies the template skeleton from
nuvel/backends/anthropic_managed_agents/templates/ into a target directory,
renames the {{agent_package}} directory, and substitutes placeholders.

Managed Agents projects are control-plane heavy: a YAML agent definition,
a YAML environment definition, a setup script that applies them once, and
a thin FastAPI server that creates sessions per request.

Usage:
    python -m nuvel.backends.anthropic_managed_agents.scaffold <agent-name>
                                                                 [--output-dir DIR]
                                                                 [--description DESC]
"""
...
def scaffold_agent(
    ...
    description = description or f"Managed Agent: {name}"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Scaffold a new Anthropic Managed Agents project."
    )
