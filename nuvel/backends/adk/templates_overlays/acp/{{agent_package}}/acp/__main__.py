...
way any stray ``print`` from the agent, plugins, or dependencies can't
corrupt the JSON-RPC stream. All logging goes to stderr too.
"""
...
def main() -> None:
    ...
    logging.basicConfig(
        stream=sys.stderr,
        level=logging.getLevelName(__import__("os").getenv("LOG_LEVEL", "WARNING")),
    )
