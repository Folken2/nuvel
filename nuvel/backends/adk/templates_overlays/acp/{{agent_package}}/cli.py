...
It reuses the same ``AgentHarness``-wired Runner as the FastAPI server and
the ACP adapter, so tools, plugins, memory, and cost tracking all behave
identically. For the Agent Client Protocol (editor integration) use
``python -m {{agent_package}}.acp`` instead.

Set ``DEV_MODE=true`` for in-memory sessions locally (no Postgres needed).
"""
