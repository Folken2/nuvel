"""
Pure helpers shared by the meta-agent's memory forks.

Owns the conversation-snapshot formatter and the tool-whitelist callback
factory used to gate a throwaway judge agent's toolset. No I/O, no ADK runner
plumbing (that lives in ``sibling_runner.py``) — kept dependency-free so it is
trivially unit-testable. Mirror of the generated-agent ``fork_utils``.
"""
