"""
Per-session throttle for the meta-agent's after-turn "fork" callbacks.

Mirror of the generated-agent throttle (``backends/adk/templates/.../memory/
throttle.py``) for the meta-agent's own judge fork. ``try_claim`` guards each
after-turn fork with two limits kept in session state, keyed per fork type:
