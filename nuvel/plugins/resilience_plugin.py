...
Runs early in the plugin chain (before security/cache) to fail fast
...
# Tools protected by the circuit breaker (configurable via env var)
_PROTECTED_TOOLS = set(
    t.strip()
    for t in os.getenv("PROTECTED_TOOLS", "scaffold_agent,validate_agent").split(",")
    if t.strip()
)
